...
export default function EducationSection({ data }: EducationSectionProps) {
  ...
  const timelineItems = educations.map((edu, index) => ({
    ...
    children: (
      <div 
        className="space-y-4 group transition-all duration-500 hover:scale-[1.02] hover:shadow-lg p-4 rounded-lg hover:bg-white border border-transparent hover:border-blue-200"
        style={{ animationDelay: `${index * 200}ms` }}
      >
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2">
          <div className="space-y-1">
            <Title level={4} className="!mb-0 text-gray-800 transition-all duration-300 group-hover:text-blue-600 group-hover:scale-105">
              {edu.schoolName}
            </Title>
            <div className="flex flex-wrap items-center gap-2">
              <Tag 
                color={getDegreeColor(edu.degree)} 
                className="font-medium transition-all duration-300 hover:scale-110 hover:shadow-md cursor-default"
              >
                {edu.degree}
              </Tag>
              <Text className="text-blue-600 font-medium transition-all duration-300 group-hover:text-blue-800 hover:scale-105">
                {edu.major}
              </Text>
            </div>
          </div>
          
          <div className="flex items-center gap-2 text-sm text-gray-600 transition-all duration-300 hover:text-blue-600 hover:scale-105">
            <CalendarOutlined className="transition-transform duration-300 hover:rotate-12" />
            <Text className="transition-colors duration-300 hover:text-blue-600">
        ...
        <div className="bg-gray-50 p-4 rounded-lg space-y-3 transition-all duration-300 group-hover:bg-gradient-to-r group-hover:from-blue-50 group-hover:to-purple-50 group-hover:shadow-inner">
          ...
          <div className="flex items-center gap-4 transition-all duration-300 hover:bg-white hover:shadow-sm rounded p-3 -m-3">
            <div className="flex items-center gap-2">
              <TrophyOutlined className="text-yellow-600 transition-all duration-300 hover:scale-125 hover:rotate-12 hover:text-yellow-500" />
              <Text strong className="text-sm transition-colors duration-300 hover:text-yellow-600">GPA:</Text>
            </div>
            
            <div className="flex-1 max-w-xs">
              <div className="flex items-center gap-2 mb-1">
                <Text strong className="text-lg text-blue-600 transition-all duration-300 hover:text-blue-800 hover:scale-110">
                  {edu.gpa.toFixed(2)}
                </Text>
                <Text className="text-sm text-gray-600 transition-colors duration-300 hover:text-gray-800">/10.0</Text>
                <Tag 
                  color={getGpaColor(edu.gpa)} 
                  className="text-xs transition-all duration-300 hover:scale-110 hover:shadow-md animate-pulse"
                >
                  {getGpaLabel(edu.gpa)}
                </Tag>
              </div>
          ...
          {edu.description && (
            <div className="flex items-start gap-2 transition-all duration-300 hover:bg-white hover:shadow-sm rounded p-3 -m-3">
              ...
              <div className="flex-1">
                ...
                <Text className="text-gray-700 text-sm leading-relaxed transition-colors duration-300 hover:text-gray-900">
                  {edu.description}
                </Text>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500 transition-all duration-300 hover:bg-white hover:shadow-sm rounded p-2 -m-2 hover:text-blue-500">
            <CalendarOutlined className="transition-transform duration-300 hover:scale-125" />
            <Text className="transition-colors duration-300 hover:text-blue-500">
              Thời gian học: {(() => {
